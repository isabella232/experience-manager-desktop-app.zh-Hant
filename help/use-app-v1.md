---
title: 使 [!DNL Experience Manager] 用案頭應用程式1.x版。
...
Using the App, the assets within [!DNL Experience Manager] are easily accessible on your local desktop and can be used in any desktop applications. 您可在Mac Finder或Windows檔案總管中輕鬆顯示資產、在案頭應用程式中開啟並在本機變更——這些變更會儲存回儲存庫中 [!DNL Experience Manager] 建立的新版本。

此類整合可讓組織中的不同角色集中管理資產，並在Creative Cloud和其他應用程式中存取資產，同時輕鬆符合包括品牌在內的各種標準。

您使用案頭應用程式v1所做 [!DNL Experience Manager] 的主要工作包括：
...
如需各種建議的dos和don&#39;ts，請參閱使 [用應用程式的最佳實務](best-practices-for-v1.md)。 如果您使用應用程式時遇到問題，請參閱如何疑難排 [ [!DNL Experience Manager] 解案頭](troubleshoot-app-v1.md)。

>[!NOTE]
>
>[!DNL Experience Manager] 案頭應用程式已在 [!DNL Experience Manager] 6.1版中推出，稱為 [!DNL Experience Manager Assets Companion App]。
...
## 安裝應用程式並將其連接至伺 [!DNL Experience Manager] 服器 {#installandconnect}

在您開始建立或編輯創意資產之前，請先將案頭應用程式與伺服器連 [!DNL Assets] 接，以下載並上傳儲存庫中的資產。 執行下列工作：

1. [安裝應用程式](#installapp)。
1. [設定您的偏好設定](#inapppref) 和連線詳細資訊。
1. [連線至伺 [!DNL Experience Manager] 服器](#connect) ，並將資產存放庫裝載為本機磁碟。
1. [在伺服器上啟用案頭動作](#desktopactions)[!DNL Experience Manager] 。

[!DNL Experience Manager] 案頭應用程式使用HTTPS連線來連線至伺 [!DNL Experience Manager] 服器，以強穩且安全地傳輸您的資產。

>[!NOTE]
>
>對於部分或全部的安裝和配置步驟，可能需要管理員或系統管理員 [!DNL Experience Manager] 的幫助。

### 安裝應用程式 {#installapp}

若要使用 [!DNL Experience Manager] 案頭應用程式，請確定您的 [!DNL Experience Manager] 伺服器版本受應用程式支援。 下載適合您作業系統（Mac或Windows）的安裝檔（二進位），並安裝應用程式。

根據您的網路和系統首選項，可能需要進行詳細配置。 如需詳 [細資訊，請 [!DNL Experience Manager] 參閱安裝及設定案頭應用程式](install-configure-app-v1.md) 。

1. 前往案頭應 [[!DNL Experience Manager] 用程式下載頁面](https://helpx.adobe.com/experience-manager/kb/download-companion-app.html) ，並下載適合您作業系統的二進位檔。
...
應用程式可讓設定連線與伺服器斷開連 [!DNL Experience Manager] 線、檢視上傳狀態、管理本機快取等。 預設設定適用於應用程式的一般使用者。 您可以調整設定，讓應用程式和伺服器整合發揮更大 [!DNL Experience Manager] 效益。 下面將詳細說明各種設定。

**瀏覽資產** ：開啟掛載儲存庫的本 [!DNL Assets] 地驅動器。 換言之，請探索您本機電腦上現在可用的資產。

**檢視資產狀態** ：當變更的資產上傳或新資產新增至儲存庫時，應用 [!DNL Assets] 程式會在背景上傳資產。 背景上傳可讓作業更順暢，您不必等到上傳完成，尤其是大型資產。 您可以將變更儲存在本機，而不必擔心。 應用程式會花一些時間將這些資產傳送至伺服器，視可用頻寬而定。 您可以檢查上傳的狀態，以及一些更基本的資訊。

**選項** ：從案頭應用程式托盤按一下選項，以存取設定，在您的系統啟動時啟動應用程式；在應用程式 [!DNL Experience Manager] 啟動時連線至伺服器；並更改安裝後可用的本 [!DNL Assets] 地驅動器號。

**「進階>管理快取** 」您可以控制可用於本機快取的磁碟空間量。 伺服器中的物件 [!DNL Assets] 會在本機快取，以提供更順暢的體驗。 您可以變更預設值以符合您的需求。 此外，您也可以清除快取，重新擷取所有資產。 清除快取時，它會保留未儲存的變更。 任何未簽入伺服器 [!DNL Experience Manager] 的資產都將保留並不刪除。

### 連接至服務 [!DNL Experience Manager] 器 {#connect}
...
1. 啟動桌 [!DNL Experience Manager] 面應用程式。 若要將您的 [!DNL Experience Manager] 例項與應用程式對應，請以 [!DNL Experience Manager] 格式指定您的伺服器 `https://[aem-server-url]:[port]`。

   ![在Mac上驗證並提供伺服 [!DNL Experience Manager] 器URL](assets/aem_desktop_app_server_url.png)

1. 在登入畫面中，指定您例項的使用者名稱和密碼。 要指定備用實 [!DNL Experience Manager] 例，請選擇 **[!UICONTROL Alternate Login URL]** 選項。

   ![在桌 [!DNL Experience Manager] 面應用程式的登入畫面上提供伺服 [!DNL Experience Manager] 器認證](assets/login_screen_v1.png)

### 在網頁介面中啟用桌 [!DNL Experience Manager] 面動作 {#desktopactions}
...
在Mac上，從內容選 **單選取** 「開啟」，以透過案頭應用程式 [!DNL Experience Manager] 開啟資產。 在Windows上，從上下文選單選擇「在Web上開啟」以開啟資產。 在「資產狀態」視窗中，按一下／點選「在桌 ![面上開啟」圖示](assets/do-not-localize/aemassets_icon_openondesktop.png) ，以開啟資產。
...
>[!NOTE]
>
>在Windows上，預 [設的Windows 7設定](https://support.microsoft.com/en-us/kb/2668751) , [!DNL Experience Manager] 會防止案頭應用程式處理大於50 MB的資產。

>[!NOTE]
>
>Adobe建議您前往Mac上的Finder檢視選項，並停用已載入資料夾的 **「顯示項目資訊**」、「顯示項目預覽 **」和「**&#x200B;顯示預覽」 ****[!DNL Assets] 欄。 它改善了效能。

### 介面中的其他選 [!DNL Experience Manager] 項 {#additional-options-in-aem-assets}

將儲存庫映 [!DNL Assets] 射到本地驅動器後，可以為映射的資產和資料夾啟用附加表徵圖和資料夾上載功能。

1. 開啟介 [!DNL Assets] 面並將指標暫留在資料夾或資產上，以在「卡片」檢視中將案頭動作顯示為快速動作。
...
### 從網頁介面檢視 [!DNL Experience Manager] 資產 {#check-out-assets-from-the-aem-web-interface}

[!DNL Assets] 可讓您取出要編輯的資產，並在完成變更後將其存回。 結帳資產後，只有您可以編輯、註解、發佈、移動或刪除資產。 取出資產會鎖定資產，並防止其他使用者執行其中任何操作。 若要能夠登出／登入資產，您需要有其「寫入」存取權。

從Web介面簽出資產有兩種 [!DNL Experience Manager] 方式。 如需有關第一種方法的詳細資訊，請參 [閱「資產UI」的登入和結帳檔案](https://experienceleague.adobe.com/docs/experience-manager-65/assets/managing/check-out-and-submit-assets.html)。 請依照下列步驟，取得第二種方法，以在安裝案頭應用程式時登出 [!DNL Experience Manager] 並開啟資產。
...
1. 若要在Adobe Creative Cloud應用程式中開啟資產以進行編輯，請按一下／點選「編輯案頭快速動作編輯案頭」 ![圖示](assets/do-not-localize/aemassets_icon_editdesktop.png)。 此外，也會檢查資產以進行編輯。 編輯完成後，請簽入資產以更新中的變更 [!DNL Assets]。
...
資產會上傳至伺 [!DNL Experience Manager] 服器。 （可選）您可以從系統托盤表徵圖中選擇「查看 **資產狀態** 」來檢查上載狀態。 或者，您也可以從Web介面簽入 [!DNL Experience Manager] 資產。 按一下已勾選的資產或加以選取。 在工具列中，按一下登入圖示 ![登入圖示](assets/do-not-localize/aemassets_icon_checkin.png)。

在本機儲存任何變 [!DNL Experience Manager] 更後，資產會自動上傳至。 此登入功能可讓其他使用者使用資 [!DNL Experience Manager] 產進行編輯。

### 大量上傳資產和資料夾至伺 [!DNL Experience Manager] 服器 {#bulkupload}

使用 [!DNL Experience Manager] 案頭應用程式，您可以將包含資產的整個檔案夾從本機檔案目錄上傳至 [!DNL Assets]。 如此，資料夾內的所有資產都會大量上傳，而不必一次上傳一個資產。
...
或者，在Finder或檔案總管中選取您要上傳至的檔案／檔案夾、將檔案複製至系統剪貼簿、導覽至網路共用區中的目標檔案夾，並從案頭應用程式內容選單中選取「 [!DNL Experience Manager] 貼上資產」 [!DNL Experience Manager]****。 如此，案頭 [!DNL Experience Manager] 應用程式就會開始上傳貼上的資產，類似於網頁介面中 **的「上傳資料夾** 」選項 [!DNL Experience Manager] 。
