...
* [連線AEM伺服器](#installandconnect)

* [直接在案頭上開啟資產](#openondesktop)
* [從案頭編輯及取出資產](#workonassets)

* [大量上傳資產和檔案夾](#bulkupload)

如需各種建議的dos和don'ts，請參閱使 [用應用程式的最佳實務](best-practices-for-v1.md)。 如果您使用應用程式時遇到問題，請參閱如何疑難排 [解AEM Desktop](troubleshoot-app-v1.md)。
...
AEM Desktop應用程式與AEM Assets整合在您的創意工作流程中，並提供下列觸點。
...
在瀏覽器的「資產」UI中，您可以探索資產位置或結帳並開啟資產，以便在案頭應用程式中編輯。 這些選項稱為「案頭動作」，預設為未啟用。 請依照下列步驟進行啟用。

1. 在「資產」主控台中，按一下／點選工 **具列中的** 「使用者」圖示。
...
### 從AEM網頁介面顯示已映射資產的位置 {#reveal-the-location-of-mapped-assets-from-aem-web-interface}
...
1. 開啟AEM Assets介面，並將指標暫留在資料夾或資產上，以在「卡片」檢視中將案頭動作顯示為快速動作。
   ...
   在「資產」UI中，開啟快速動作功能表以檢視案頭動作

   當您在選取資產後，或從資產頁面的工具列按一下／點選工具列中的「案頭動作 **** 」圖示時，這些案頭動作也可供使用。

1. 若要在案頭應用程式中開啟與特定副檔名相關聯的資產，請按一下／點選「在案頭上開啟」快速動作「在案頭上 **開啟」圖示**![](assets/aemassets_icon_openondesktop.png)。

   或者，從工具 **欄的** 「案頭動 **作** 」選單選擇「開啟」。

1. 按一下／點選「顯 **示** 」快速動作「 ![顯示」圖示](assets/aemassets_reveal_icon.png) ，以找出本機檔案系統上的特定資產。

   或者，從工具 **欄的** 「案頭 **操作** 」菜單中選擇「顯示」。

### 從Finder或Explorer開啟AEM資產 {#open-aem-assets-from-the-finder-or-the-explorer}

在Mac上，從內容選單選取「開啟」，以透過AEM Desktop開啟資產。
...
在Windows上，從上下文選單選擇「在Web上開啟」以開啟資產。 在「資產狀態」視窗中，按一下／點選「在桌 ![面上開啟」圖示](assets/aemassets_icon_openondesktop.png) ，以開啟資產。
...
從AEM網頁介面簽出資產有兩種方式。 如需有關第一種方法的詳細資訊，請參 [閱「資產UI」的登入和結帳檔案](https://helpx.adobe.com/in/experience-manager/6-4/assets/using/check-out-and-submit-assets.html)。 請依照這些步驟進行，以取得在安裝AEM Desktop應用程式時，要簽出並開啟資產的第二種方法。
...
### 查看Mac上的資產 {#check-out-assets-on-mac}
...
1. 在「資產資訊」對話方塊中，按一下／點選「結帳」圖示以結帳資產。 按一下／點選「簽出」表徵圖後，「簽入」表徵圖將切換為「簽入」表徵圖。

   ![瀏覽至要結帳的資產](assets/chlimage_1-6.png)

1. 若要簽入資產，以便其他用戶可以使用，請按一下／按一下「資產資訊」對話框中的「簽入」表徵圖。
...
1. 在「資產資訊」對話方塊中，按一下／點選「結帳」圖示。 「檢出」(Checkout)表徵圖切換為「檢入」(Checkin)表徵圖。
...
1. 在Explorer中檢閱資產。 資產「資產」鎖定圖示上的 ![鎖定圖示](assets/aemassets_icon_lockcheckout.png) ，表示您已簽出資產。
   ...
   >[!NOTE]
   >延遲幾分鐘後，可能會出現鎖定圖示。 AEM Desktop應用程式會快取資產以供快速存取，因此可能需要片刻時間來更新鎖定的狀態。

1. 若要簽入資產，以便讓其他用戶可以使用，請按一下／按一下「資產資訊」( **Asset Info** )對話框中的「簽入」(Checkin)表徵圖。

### 使用Finder或Explorer並使用Web介面簽入資產 {#check-in-an-asset-using-finder-or-explorer-and-using-web-interface}

編輯完資產後，請將資產儲存在案頭應用程式中。 從上下文菜單中，選擇「更多資產資訊」，然後按一下／點選「簽入」。

資產會上傳至AEM伺服器。 （可選）您可以從托盤表徵圖中選擇「查看資產狀態」來檢查上載狀態。

![AEM案頭應用程式檔案傳輸和上傳狀態視窗](assets/aem_desktopapp_upload_status.png)

或者，您也可以從AEM網頁介面簽入資產。 按一下／點選已勾選的資產或加以選取。 在工具欄中，按一下／點選檢入表徵圖「檢入」( ![Checkin)表徵圖](assets/aemassets_icon_checkin.png)。
...
請注意，不建 *議您從Finder或Explorer的本機磁碟複製並貼上* ，或將大量檔案／巢狀資料夾拖放至AEM案頭應用程式映射的網路共用區。 它比上述的「上傳資料夾」功能可靠得多。

如果您偏好在案頭上工作，另一種選擇是在Finder或檔案總管中選取您要上傳至AEM的檔案／檔案夾、將檔案複製至系統剪貼簿，然後導覽至網路共用區域的目標檔案夾，並從AEM案頭應用程式內容選單中選取「貼上資產」。 這樣，AEM案頭應用程式就會開始上傳貼上的資產，類似上述的「上傳檔案夾」。
...
>* [疑難排解AEM Desktop應用程式](troubleshoot-app-v1.md)
